...
import { myAddEvent, myRemoveEvent } from '_utils';
/**
 * Layer组件
 *
 * @param [title] 标题
...
class Layer extends React.Component {
    constructor(props) {
        super(props);
        const { display, width, height, animation } = props;
    ...
    componentDidMount() {
        ...
        myAddEvent(window, 'scroll', this._handleScroll);
    }
    ...
    layerClose = () => {
        ...
        if (shadeClose) {
            myRemoveEvent(this.divEL, 'click', this._handleCloseClick);
        }
        if (isFullScreen) {
    ...
    _maskLayer = () => {
        ...
        if (shadeClose) {
            myAddEvent(this.divEL, 'click', this._handleCloseClick);
        }
    };
    ...
    render() {
        const { width, height, isFullScreen, display, left, top, contentHeight, animation, docEle } = this.state;
...
Layer.propTypes = {
    ...
    openCallBack: PropTypes.string,
    closeCallBack: PropTypes.string,
